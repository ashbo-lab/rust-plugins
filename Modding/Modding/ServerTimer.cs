...
namespace Oxide.Plugins
{
    ...
    class ServerTimer : RustPlugin
    {
        ...
        public void startTimer()
        {
            ...
            if (days > 1)
            {
                startDaysTimer(days, hours, minutes);
                return;
            }
            else if (hours > 1)
            {
            ...
            MinutesTimer = timer.Every(10, () =>
{
    ...
    foreach (var player in BasePlayer.activePlayerList)
    {
        SendReply(player, $"{minutes}:{_config.date.Subtract(DateTime.Now).Seconds}");
    ...
    send++;
        ...
        void startDaysTimer(int days, int hours, int minutes)
        {


            foreach (var player in BasePlayer.activePlayerList)
            {
                SendReply(player, $"{days} days and {hours}:{minutes} h till Purge!");
            }
            int send = 0;
            DaysTimer = timer.Every(DaySeconds / 2, () =>
              {

                  days = _config.date.Subtract(DateTime.Today).Days;
                  hours = _config.date.Subtract(DateTime.Now).Hours;
                  ...
                  if (days <= 1)
                  {
                      foreach (var player in BasePlayer.activePlayerList)
                      {
                          SendReply(player, $"{days} days and {hours}h till Purge left");
                      }
                      startHoursTimer();
        ...
        private void gui(BasePlayer player)
        {
            ...
            var mainName = elements.Add(new CuiPanel
            {
                ...
                RectTransform = { AnchorMin = "0.94 0.92", AnchorMax = "0.99 0.99" },
                CursorEnabled = false
            }
          , "Overlay", "Timer");
