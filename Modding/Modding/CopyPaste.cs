...
{
    [Info("Copy Paste", "Reneb & MiRror & Misstake & misticos & Oegge", "4.1.21")]
        ...
        private void PasteLoop(PasteData pasteData)
        {
            ...
            foreach (var data in todo)
            {
                ...
                if (containerIo != null)
                {
                    ...
                    if (autoTurret != null)
                    {
                        if (autoTurret.GetAttachedWeapon() == null)
                        {
                            var ak = ItemManager.CreateByItemID(1545779598, 1, 0);
                            var ammo = ItemManager.CreateByItemID(-1211166256, 250, 0);
                            autoTurret.inventory.itemList.Add(ak);
                            autoTurret.inventory.itemList.Add(ammo);
                            autoTurret.OnItemAddedOrRemoved(ak, true);
