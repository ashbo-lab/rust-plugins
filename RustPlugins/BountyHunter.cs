...
{
    [Info("BountyHunter", "Oegge & ashbo", "2.0.0")]
    ...
    class BountyHunter : CovalencePlugin
    {
        ...
        private class Mission : IComparable
        {
            public ulong victim;
            public ulong client;
            public int reward;
            ...
            public int CompareTo(object obj) {
                ...
                int otherReward = otherMission.reward;
                if (otherReward < this.reward) return -1;
                if (otherReward > this.reward) return 1;
                return 0;
            }
        }
        ...
        // place a reward of "reward" Gold on the death of the given "victim"
        // if a reward is placed on a victim the same player already placed a bounty on, the two missions will be merged together.
        private void cmdPlaceBounty(IPlayer iclient, IPlayer victim, int reward)
        {
            ...
            bool adding = false;
            foreach (Mission old in data.missions)
            {
                if ((old.client ==ulong.Parse(iclient.Id)) && old.victim == ulong.Parse(victim.Id))
                {
                    players.FindPlayer(player.UserIDString).Reply("have placed a second bounty on someone");
                    old.reward += reward;
                    adding = true;
                }
            }
        ...
        private void cmdRemoveBounty(IPlayer iclient, IPlayer victim)
        {
            ...
            {
                if (mission.client == player.userID && mission.victim == ulong.Parse(victim.Id))
                {
                    ok = true;
                    removeM.Add(mission);
                    ...
                    if (data.deadpool[ulong.Parse(victim.Id)].Count == 0)
                    {
                        ...
                        foreach (ulong hunterID in data.hunts.Keys)
                        {
                            ...
                            data.hunts[hunter.userID].Remove(ulong.Parse(victim.Id));
            ...
            if (!ok)
            {
                players.FindPlayer(player.UserIDString).Reply("There is no active bounty placed on " + victim.Name + " by you");
            }
            else
            {
                players.FindPlayer(player.UserIDString).Reply("All bounties you had set on " + victim.Name + " have been removed succesfully");
            }
            SaveData();
        }
        ...
        //Sends a list of all active bounty stations with their grid coords as a chat message
        ...
        //Sends a list of all players with bounties on them and the total reward for their death as a chat message
        ...
        private void hunted(BasePlayer hunter, BasePlayer victim) {
            int gold = getTotalReward(covalence.Players.FindPlayerById(victim.userID.ToString()));
        ...
        private String listDeadpool() {
            ...
            foreach (ulong playerID in data.deadpool.Keys) {
                list += (BasePlayer.FindByID(playerID).displayName + ": " + getTotalReward(covalence.Players.FindPlayerById(playerID.ToString())) + " gold" + "\n");
            }
            return list;
        }
        ...
        private  float AnchorMin1 = 0.01f;
        private float AnchorMin2start = 0.86f;
        private float AnchorMax2Start = 0.9f;
        private float AnchorMax1 = 0.7f;


        private void createheader(String name, CustomCuiElementContainer elements, String mainName)
        {
            ...
            elements.Add(new CuiLabel
            {
                ...
                RectTransform = { AnchorMin = "0.01 0.92", AnchorMax = "0.99 0.99" },
                Text = { Text = name, FontSize = 25, Align = TextAnchor.MiddleCenter }
            }, mainName);
        }
        ...
        private void createBountyList(int page, IPlayer iplayer, CustomCuiElementContainer elements, List<Mission> missions, String mainName)
        {
            ...
            createheader("My Bounties", elements, mainName);
        ...
        private void createAllBountyList(int page, IPlayer iplayer, CustomCuiElementContainer elements, String mainName)
        {
            #region header
            createheader("All Bounties", elements, mainName);
            #endregion

            List<Mission> missions;
            var keys = data.deadpool.Keys.ToList();

            if (keys.Count > 0)
            {
                #region Fields
                float step = 0f;
                List<List<ulong>> keyslists = new List<List<ulong>>();
                int Pages = (int)Math.Ceiling((double)keys.Count / 13);
                int start = 0;
                int last = keys.Count;
                ...
                {
                    int dist = (start + 13 <= last) ? 13 : (last - start);
                    keyslists.Add(keys.GetRange(start, dist));
                    start += 13;
                }
                #endregion


                foreach (var key in keyslists[page])
                {
                    var victim = covalence.Players.FindPlayerById(key.ToString());
                    var reward = getTotalReward(victim);


                    System.Console.WriteLine(victim.Name + ": " + reward);
                    #region Names
                    ...
                    elements.Add(new CuiLabel
                    {
                        ...
                        RectTransform = {
                            AnchorMin = $"0.75 {AnchorMin2start - step}"
                            , AnchorMax = $"1 {AnchorMax2Start - step}"
                        },
                        Text = {
                            Text = "",
                            FontSize = 16,
                    ...
                    elements.Add(new CuiButton
                    {
                        Button = { Command = $"acceptBounty {victim.Id.ToString()}", Color = ButtonColour },
                        RectTransform = { 
                            AnchorMin = $"0.75 {AnchorMin2start - step}", 
                            AnchorMax = $"0.99 {AnchorMax2Start - step}" },
                        Text = { Text = "accept Bounty", FontSize = 14, Align = TextAnchor.MiddleCenter }
                    }, mainName);
                    ...
                    if (!keyslists.Last().Equals(keyslists[page]))
                        elements.Add(new CuiButton
                        {
                            Button = { Command = $"AllBounty {"AllList"} {page + 1}", Color = ButtonColour },
                            RectTransform = { AnchorMin = "0.8 0.12", AnchorMax = "0.9 0.15" },
                            Text = { Text = "Next", FontSize = 15, Align = TextAnchor.MiddleCenter }
                        }, mainName);
        ...
        private String guiString = "0 0 0 0.5";
        ...
        void MainUI(BasePlayer player)
        {
            ...
            elements.Add(new CuiButton
            {
                Button = { Command = $"AllBounty {"MenuGUI"}", Color = ButtonColour },
        ...
        [Command("AllBounty")]
        private void allBountyPage(IPlayer iplayer, String command, string[] args)
        {
            #region Start
            ...
            createAllBountyList(page, iplayer, elements, mainName);
            ...
            #region Close and Back Button

            
            //CloseButton
            elements.Add(new CuiButton
            {
                Button = { Command = $"ClosePM {"AllList"}", Color = ButtonColour },
            ...
            elements.Add(new CuiButton
            {
                Button = { Command = $"GUI {"AllList"}", Color = ButtonColour },
        ...
        private void removeBountyCallBack(IPlayer iplayer, String command, string[] args)
        {
            ...
            myBountyPage(iplayer, null, new string[] { "BountyList" });
        }
        #endregion
        ...
        [Command("acceptBounty")]
        private void acceptBountyCallBack(IPlayer iplayer, String command, string[] args)
        {
            cmdAcceptBounty(iplayer, covalence.Players.FindPlayerById(args[0]));
        
        }
        #endregion acceptBounty
