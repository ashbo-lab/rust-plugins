...
{
    [Info("BountyHunter", "Oegge & ashbo", "1.0.0")]
    ...
    class BountyHunter : RustPlugin
    {
        ...
        private class ConfigFile
        {
            ...
            public List<String> allowedStationItems;
            public static ConfigFile DefaultConfig()
            {
                List<String> items = makeDefaultStations();
            ...
            private static List<String> makeDefaultStations() {
                List<String> stations = new List<String>();
                stations.Add("dropbox.deployed");
                stations.Add("box.wooden.large");
                stations.Add("mailbox.deployed");
                stations.Add("woodbox_deployed");
                stations.Add("wall.frame.shopfront.metal");
                stations.Add("fridge.deployed");
                stations.Add("coffinstorage");
                return stations;
            }
        }
        ...
        ItemContainer.CanAcceptResult? CanAcceptItem(ItemContainer container, Item item, int targetPos)
        {
            ...
            if (!data.stations.Contains(container.entityOwner.ToString()))
                return null;
            if (item.name == null) {
                return null;
            }
        ...
        [ChatCommand("bounty.place")]
        private void cmdPlaceBounty(BasePlayer player, String command, string[] args)
        {
        ...
        [ChatCommand("bounty.remove")]
        private void cmdRemoveBounty(BasePlayer player, String command, string[] args)
        {
            if (args.Length != 1)
            {
                SendReply(player, "Invalid Command");
        ...
        [ChatCommand("bounty.accept")]
        private void cmdAcceptBounty(BasePlayer player, String command, string[] args)
        {
            if (args.Length != 1)
            {
                SendReply(player, "Invalid Command");
...
//sysout            
            ...
            if (!config.allowedStationItems.Contains(entity.ShortPrefabName)) { 
                SendReply(player, "The item in your line of sight may not be chosen as Bounty station");
                return;
            }

            if (!data.stations.Contains(entity.ToString()))
            {
                data.stations.Add(entity.ToString());
        ...
        private void cmdReset(BasePlayer player, String command, string[] args)
        {
            ...
            data.stations = new HashSet<string>();
            SaveData();
