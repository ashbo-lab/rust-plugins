...
{
    [Info("BountyHunter", "Oegge & ashbo", "1.2.0")]
    ...
    class BountyHunter : CovalencePlugin
    {
        ...
        private void cmdPlaceBounty(ConsoleSystem.Arg arg) {
            ...
            {
                players.FindPlayer(player.UserIDString).Reply("Invalid reward");
                return;
            }
        ...
        private void cmdAcceptBounty(ConsoleSystem.Arg arg)
        {
             ...
             * foreach (Mission mission in data.deadpool[victim.userID]) {
                if (mission.client == player.userID) { 
                    players.FindPlayer(player.UserIDString).Reply("You can't accept a bounty hunt you're set yourself");
                    return;
                }
            }
            */
