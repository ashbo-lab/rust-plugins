...
namespace Oxide.Plugins
{
    ...
    class BountyHunter : CovalencePlugin
    {
        ...
        private void cmdPlaceBounty(IPlayer iclient, IPlayer ivictim, int reward) {
            ...
            BasePlayer victim = BasePlayer.FindByID(ulong.Parse(ivictim.Id));
            ...
            players.FindPlayer(player.UserIDString).Reply("Your have succesfulls placed a bounty of " + reward + " gold on " + victim._name);
        ...
        // former [ConsoleCommand("bounty.remove")]
        private void cmdRemoveBounty(IPlayer iclient, IPlayer ivictim)
        {
            BasePlayer player = BasePlayer.FindByID(ulong.Parse(iclient.Id));
            BasePlayer victim = BasePlayer.FindByID(ulong.Parse(ivictim.Id));
            ...
            if (!data.deadpool.Keys.Contains(victim.userID))
            {
                players.FindPlayer(player.UserIDString).Reply("There is no bounty on " + victim._name);
                return;
            }
        ...
        private void cmdAcceptBounty(IPlayer iclient, IPlayer ivictim)
        {
            ...
            if (!data.deadpool.ContainsKey(victim.userID))
            {
                players.FindPlayer(player.UserIDString).Reply("There are no bounties set on " + victim.displayName);
                return;
            }
            ...
            players.FindPlayer(player.UserIDString).Reply("You are on the hunt for " + victim.displayName);
            players.FindPlayer(player.UserIDString).Reply("You may now attack " + victim.displayName + " on the whole map. Be aware that " + victim.displayName + " will be allowed to defend");
            SaveData();
        }
        ...
        #region GUIHelper
        private Dictionary<string, string> UserInputPlayerSearch = new Dictionary<string, string>(); // Format: <userId, text>
        ...
        private String guiString = "0 0 0 0";
        ...
        /*
        [Command("Close")]
        private void close(IPlayer iplayer, String command, string[] args)
        {
        ...
        */
        ...
        private void availablePlayers(IPlayer iplayer, String command, string[] args)
        {
            ...
            int page = args.Length > 1 ? int.Parse(args[1]) : 0;
            ...
            String guiString = String.Format("0 0 0 {0}", 0.5);
            #endregion
        ...
        private void inputfieldControll(IPlayer iPlayer, String command, string[] args)
        {
            ...
            if (iPlayer.IsServer  || args.Count() <= 0)
            {
                if (UserInputPlayerSearch.ContainsKey(iPlayer.Id))
                    UserInputPlayerSearch.Remove(iPlayer.Id);
            ...
            if (UserInputPlayerSearch.ContainsKey(iPlayer.Id))
            {
                System.Console.WriteLine("args0"+args[0]);
                UserInputPlayerSearch[iPlayer.Id] = args[0];
            }
            else
            {
                UserInputPlayerSearch.Add(iPlayer.Id, args[0]);
            }


            foreach(String k in UserInputPlayerSearch.Keys)
            System.Console.WriteLine("key: "+ k+" value: "+UserInputPlayerSearch[k]);
        ...
        private void addBounty(IPlayer iplayer, String command, string[] args)
        {
            ...
            if (!UserInputPlayerSearch.ContainsKey(iplayer.Id))
                return;
            var bounty = UserInputPlayerSearch[iplayer.Id];
            System.Console.WriteLine(bounty);
            System.Console.WriteLine("args0 "+args[0]);
